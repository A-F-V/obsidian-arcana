export function escapeCurlyBraces(text: string): string {
  return new String(text).replace(RegExp('{', 'g'), '{{').replace(RegExp('}', 'g'), '}}');
}
...
export function isEmoji(text: string): boolean {
  const emojiRange = /[\u{1F300}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F1E6}-\u{1F1FF}]/u;
